...
} from '\var\task\boss-rent-app/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\boss-rent-app/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\boss-rent-app')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\boss-rent-app',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\boss-rent-app\.netlify\dist\run\handlers\server.js')
